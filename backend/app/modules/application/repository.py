...
class ApplicationRepository:
    ...
    async def get_all(self, skip: int = 0, limit: int = 20, status: int = None, job_ids: list[int] = None, dept_ids: list[int] = None, search: str = None) -> list[tuple[JobApplication, str | None]]:
        """获取所有投递记录（员工端），可按状态/搜索过滤，返回(投递, 用户姓名)"""
        query = (
            select(JobApplication, SysUser.real_name)
    ...
    async def get_all_count(self, status: int = None, job_ids: list[int] = None, dept_ids: list[int] = None, search: str = None) -> int:
        """获取所有投递记录总数（员工端），可按状态/搜索过滤"""
        query = select(func.count(JobApplication.id)).where(JobApplication.is_deleted == 0)
        if search:
            query = (
                query.outerjoin(Resume, Resume.id == JobApplication.resume_id)
                .outerjoin(SysUser, (SysUser.id == JobApplication.user_id) & (SysUser.is_deleted == 0))
