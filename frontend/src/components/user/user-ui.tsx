...
export function EmptyState({ title, description, icon, action }: EmptyStateProps) {
  return (
    <div className="flex min-h-[18rem] flex-col items-center justify-center rounded-[2rem] border border-dashed border-sky-200 bg-white/80 px-6 py-12 text-center shadow-sm shadow-slate-200/70">
      <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-2xl bg-sky-50 text-accent" aria-hidden="true">
...
export function SectionCard({ children, title, description, action, className }: SectionCardProps) {
  return (
    <section className={cn('rounded-[1.75rem] border border-white/80 bg-white/90 shadow-sm shadow-slate-200/70 backdrop-blur', className)}>
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3" aria-label="正在加载">
      {Array.from({ length: rows }).map((_, index) => (
        <div key={index} className="h-48 animate-pulse rounded-[1.75rem] bg-white/80 shadow-sm shadow-slate-200/70" />
      ))}
    </div>
  );
}
