...
- 代码注释规范
  - 所有的函数/类/方法必须包含注释。
  - 使用当前语言的标准注释格式（如 Python 使用 Docstring，JS/TS 使用 JSDoc，Java 使用 Javadoc）。
  - 注释必须包含：功能描述、参数说明（@param）、返回值说明（@return）。
  - 对于复杂的业务逻辑，必须在关键步骤处添加行内注释，解释"为什么"这样做，而不仅是"做了什么"。

- 日志记录规范
  - 核心业务逻辑、网络请求、数据库操作必须添加日志记录。
  - 不要使用简单的 print/console.log，必须使用项目配置的日志框架（如 Python 的 logging，JS 的 winston/pino，Java 的 Log4j/SLF4J）。
  - 日志级别必须准确区分：
    - ERROR: 捕获到异常且影响主流程
    - WARN: 异常但已兜底处理，或需要关注的潜在问题
    - INFO: 关键节点的业务流转记录（如：用户登录成功、订单创建成功）
    - DEBUG: 详细的调试信息（如：函数入参、中间变量）
  - 捕获异常时，日志必须包含完整的错误堆栈信息（如：logger.error("Failed to process order", error)）。

- 生成代码时的默认行为
- 当我让你生成某个函数或模块时，默认必须带上完整的注释和日志，不需要我额外提醒。
